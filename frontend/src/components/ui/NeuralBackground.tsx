...
// ── Cyberpunk Cyan and Purple Palette ─────────────────────────────
const BG = "0,0,0"
const NODE_BRIGHT = "rgba(0, 212, 255," // Cyan active nodes
const EDGE_COLOR = "0, 212, 255" // Cyan connection edges
const PARTICLE_COLOR = "123, 97, 255" // Purple bouncing particles (stars)
...
export const NeuralBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const buildScanlines = (w: number, h: number) => {
      ...
      // Subtle cyan tint on horizontal lines
      sctx.fillStyle = "rgba(0, 212, 255, 0.008)"
      for (let y = 0; y < h; y += 4) {
    ...
    const draw = () => {
      ...
      for (const mote of dustMotes) {
        ...
        // Beautiful cyan and purple halos for floating dust particles
      ...
      packetsRef.current = packets.filter((p) => {
        ...
        // Trailing gradient from transparent to purple to cyan
        tg.addColorStop(0, "rgba(123, 97, 255, 0)")
        tg.addColorStop(0.5, "rgba(123, 97, 255, 0.08)")
        tg.addColorStop(1, "rgba(0, 212, 255, 0.16)")
        ctx.beginPath()
        ctx.moveTo(tx, ty)
        ctx.lineTo(px, py)
        ctx.strokeStyle = tg
        ...
        hg.addColorStop(0, "rgba(0, 212, 255, 0.25)")
